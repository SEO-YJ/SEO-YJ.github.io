...
Jekyll은 사이트를 빌드하는 방법을 사용자화 할 수 있는 많은 유연성을 제공한다. 이 옵션들은 정의하는 방식은 2가지로 첫째, 사이트의 루트 폴더에 위치한 "_config.yml"이나 "_config.toml" 파일에서 지정될 수 있고, 또는 터미널에 "jekyll" 실행 파일의 flags로 지정될 수 있다.
...
내 생각
1. Jekyll에서는 내가 빌드 하는 방식을 만들 수 있다.
2. 빌드하는 방식은 1) "_config.yml" 이나 "_config.toml" 파일에 지정하거나 2) "jekyll" 실행파일의 플래그에 지정하는 방식 으로 지정될 수 있다.
